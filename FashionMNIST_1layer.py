...
path_name = f'output_img/seed={seed}/Fashion_1layer/{kernel_init_type}/'
import os
...
print('训练集图片总数：', len(trainset))
print('测试集图片总数：', len(testset))
...
def build_pca_templates(class_data, pcs_per_class, size):
    ...
    for Xc in class_data:
        ...
        pc2d = merged_pc.reshape(1, size, size)    # [1, H, W]
...
class Net(nn.Module):
    def __init__(self):
        super().__init__()
        self.conv1 = nn.Conv2d(1, 10, kernel_size=target_size, padding=0, stride=1, bias=True)
...
record_mean = torch.zeros(len(classes), target_size, target_size)
record_cnt = torch.zeros(len(classes))
for i, data in enumerate(trainloader, 0):
    # get the inputs; data is a list of [inputs, labels]
    inputs, labels = data
    for i in range(len(labels)):
        record_mean[labels[i], :, :] = record_mean[labels[i], :, :] + inputs[i, 0, :, :]
...
print('GroundTruth: ', ' '.join(f'{classes[labels[j]]:12s}' for j in range(64)))



outputs = net(images)
_, predicted = torch.max(outputs, 1)
print('Predicted: ', ' '.join(f'{classes[predicted[j]]:12s}' for j in range(64)))
...
import csv
# 保存数据到CSV文件
if not os.path.exists(f"plot_data/seed={seed}/Fashion_1layer/"):
    os.makedirs(f"plot_data/seed={seed}/Fashion_1layer/", exist_ok=True)
csv_filename = f'plot_data/seed={seed}/Fashion_1layer/{kernel_init_type}.csv'
