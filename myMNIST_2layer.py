...
epoch_num = 10
path_name = f'output_img/seed={seed}/myMNIST_2layer/{kernel_init_type}/'
import os
...
# ── conv2 有意义的可视化 ──────────────────────────────────────────
# 对每个数字 i，取对应的 conv2_k*conv2_k 个输入通道，
# 在 conv2_k x conv2_k 空间上显示每个位置的有效权重
# 这样能看出训练后哪些空间位置被网络认为更重要
W2 = net.conv2.weight.data.cpu().numpy()  # [10, 90, 3, 3]
num_classes = len(classes)
fig, axes = plt.subplots(2, 5, figsize=(12, 5))
fig.suptitle(f'Conv2 effective spatial weights per digit ({kernel_init_type} init, trained)',
             fontsize=11)
for i, ax in enumerate(axes.flat):
    spatial = np.zeros((conv2_k, conv2_k))
    for xi in range(conv2_k):
        for yi in range(conv2_k):
            ch = i * conv2_k * conv2_k + xi * conv2_k + yi
            if ch < W2.shape[1]:
                spatial[xi, yi] = W2[i, ch, xi, yi]
    sns.heatmap(spatial, ax=ax, cmap='RdBu_r', center=0,
...
# 保存数据到CSV文件
if not os.path.exists(f"plot_data/seed={seed}/myMNIST_2layer/"):
    os.makedirs(f"plot_data/seed={seed}/myMNIST_2layer/", exist_ok=True)
csv_filename = f'plot_data/seed={seed}/myMNIST_2layer/{kernel_init_type}.csv'
