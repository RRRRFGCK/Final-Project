...
path_name = f'output_img/seed={seed}/SIGN_2layer/{kernel_init_type}/'
import os
...
def collect_patches_by_class_and_pos(trainloader, num_classes, patch_size, stride, grid_k=3):
    """
    返回 dict[(c,x,y)] = np.array [N, 3*patch_size*patch_size]  （RGB）
    """
    D = 3 * patch_size * patch_size
    buckets = {(c,x,y): [] for c in range(num_classes)
                          for x in range(grid_k) for y in range(grid_k)}
    cord_list = [i*stride for i in range(grid_k)]  # [0,16,32] 对应 64->32 的三格

    for imgs, labels in trainloader:               # imgs: [B,3,64,64]
        B = imgs.size(0)
        for i in range(B):
            ...
            for x in range(grid_k):
                for y in range(grid_k):
                    ...
                    patch = imgs[i, :, xs:xs+patch_size, ys:ys+patch_size]  # [3,32,32]
                    buckets[(lab,x,y)].append(patch.reshape(-1).cpu().numpy())
    ...
    for c in range(num_classes):
        for x in range(grid_k):
            for y in range(grid_k):
                ...
                templates.append(merged_pc.reshape(3, patch_size, patch_size))
...
class Net(nn.Module):
    def __init__(self):
        super().__init__()
        self.conv1 = nn.Conv2d(3, conv2_k * conv2_k * len(classes), kernel_size=conv1_k, padding=0, stride=conv1_s, bias=True)

        self.conv2 = nn.Conv2d(conv2_k * conv2_k * len(classes), len(classes), kernel_size=conv2_k, padding=0, stride=1)
...
record_mean = torch.zeros(len(classes) * conv2_k * conv2_k, 3, conv1_k, conv1_k)
    ...
    for i in range(len(labels)):
        for x_index in range(conv2_k):
            for y_index in range(conv2_k):
                ...
                record_mean[kernel_idx, :, :, :] = record_mean[kernel_idx, :, :, :] + inputs[i, :, cord_list[x_index]:cord_list[x_index] + conv1_k, cord_list[y_index]:cord_list[y_index] + conv1_k]
...
record_mean_layer2 = torch.zeros(len(classes), len(classes) * conv2_k * conv2_k, conv2_k, conv2_k)
...
import csv
# 保存数据到CSV文件
if not os.path.exists(f"plot_data/seed={seed}/SIGN_2layer/"):
    os.makedirs(f"plot_data/seed={seed}/SIGN_2layer/", exist_ok=True)
csv_filename = f'plot_data/seed={seed}/SIGN_2layer/{kernel_init_type}.csv'
