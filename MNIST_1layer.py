...
path_name = f'output_img/seed={seed}/MNIST_1layer/{kernel_init_type}/'
import os
...
def visualize_filter_bank(W, filename):
    ...
    grid = torchvision.utils.make_grid(W, normalize=True, nrow=5)  # [C, H_grid, W_grid]
...
class Net(nn.Module):
    def __init__(self):
        super().__init__()
        self.conv1 = nn.Conv2d(1, 10, kernel_size=20, padding=0, stride=1, bias=True)
...
net_weights = torchvision.utils.make_grid(record_mean, normalize=True, nrow=5)
...
import csv
# 保存数据到CSV文件
if not os.path.exists(f"plot_data/seed={seed}/MNIST_1layer/"):
    os.makedirs(f"plot_data/seed={seed}/MNIST_1layer/", exist_ok=True)
csv_filename = f'plot_data/seed={seed}/MNIST_1layer/{kernel_init_type}.csv'
