...
epoch_num   = 10
target_size = 32          # CIFAR-10 native size
in_channels = 3           # RGB
batch_size  = 64
conv1_k = 8               # conv1 kernel size: floor((32-8)/6)+1 = 5
conv1_s = 6               # stride
conv2_k = 5               # conv2 kernel size (matches 5×5 feature map)

path_name = f'output_img/seed={seed}/CIFAR10_2layer_FMinit/{kernel_init_type}/'
...
# Data
# ============================================================
# CIFAR-10 images are 32×32 RGB — no Lambda reshape needed
...
# ============================================================
# Layer-1 initialisation helpers
# ============================================================
...
def build_classmean_layer1(trainloader, num_classes, conv2_k, conv1_k, conv1_s, in_channels):
    """Per-(class, spatial-position) mean patch → [90, 3, 14, 14]"""
    cord_list = [i * conv1_s for i in range(conv2_k)]
    total = num_classes * conv2_k * conv2_k
    # NOTE: shape now includes in_channels (3 for RGB)
    W_sum = torch.zeros(total, in_channels, conv1_k, conv1_k)
        ...
        for i in range(len(labels)):
            for x in range(conv2_k):
                for y in range(conv2_k):
                    ...
                    # take all channels: imgs[i] is [3, H, W]
                    W_sum[kid] += imgs[i, :, xs:xs+conv1_k, ys:ys+conv1_k]
                    W_cnt[kid] += 1
    W = W_sum / W_cnt.view(-1, 1, 1, 1)   # [90, 3, 14, 14]
    # zero-mean + L2 norm
...
# ============================================================
# Layer-2 data-driven initialisation
# ============================================================
...
W_layer1 = build_classmean_layer1(trainloader, num_classes, conv2_k, conv1_k, conv1_s, in_channels)

# ============================================================
# Initialise network
# ============================================================
net = Net().to(device)
tbwriter = SummaryWriter(f'logs/CIFAR10_2layer_FMinit_{kernel_init_type}_seed{seed}')
...
# Filter visualisation — conv1 (3-channel filters, show mean across channels)
...
# Save CSV
# ============================================================
csv_dir = f'plot_data/seed={seed}/CIFAR10_2layer_FMinit/'
